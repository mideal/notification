...
class FakeSmsGateway implements NotificationGateway
{
    public function send(Notification $notification): GatewayResult
    {
        if (str_starts_with($notification->recipient_id, 'unavailable')) {
            throw new GatewayUnavailableException('SMS gateway is temporarily unavailable');
        }
        ...
        Log::info('SMS sent via fake gateway', [
